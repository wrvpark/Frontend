...
/**
 * Sale & Rent Component that handles creating and update sale or rent posts
 * 
...
export default function AddUpdate(props) {
    ...
    const fetchCategories = async () => {
        const response = await fetchSubCategories(localStorage.getItem("sr_cat_id"));
        try {
    ...
    const formatCategories = (data) => {
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].name === "Sale" || data[i].name === "Rent") {
                let object = {
                    value: data[i].id,
                    label: data[i].name
                };
    ...
    const title = (
        <span>
            ...
            <h1> {">"} </h1>
            <NavLink to="/saleAndRent/addupdate" exact className="currPage">
                <HeaderType />
            </NavLink>
